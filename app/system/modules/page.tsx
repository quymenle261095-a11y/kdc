...
export default function ModuleManagementPage() {
  ...
  const handleReseedModules = async () => {
    ...
    setIsReseeding(true);
    try {
      // LOGIC: Không dùng force để seeder chỉ upsert metadata, không đổi trạng thái enabled
      const modulesResult = await seedModule({ module: 'adminModules', quantity: 0 });
      if (modulesResult.errors?.length) {
        throw new Error(modulesResult.errors.join(', '));
      }

      const presetsResult = await seedModule({ module: 'systemPresets', quantity: 0 });
