...
export default function IntegrationsPage() {
  ...
  React.useEffect(() => {
    ...
    const nextForm: AiForm = {
      adminDirectAiEnabled: aiConfig.adminDirectAiEnabled ?? false,
      apiKey: '',
  ...
  const handleSaveAi = async () => {
    ...
    try {
      ...
      await saveAiConfig({
        adminDirectAiEnabled: aiForm.adminDirectAiEnabled,
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm space-y-6">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <label className="flex items-center gap-2 rounded-full border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300" title="Bật/Tắt khối ChatJPT trực tiếp trong các modal Import AI ở Admin">
                    <input
                      type="checkbox"
                      checked={aiForm.adminDirectAiEnabled}
                      onChange={(e) => updateAiField('adminDirectAiEnabled', e.target.checked)}
                      className="h-4 w-4 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500"
                    />
                    ChatJPT trong Import Admin
                  </label>
                </div>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  >
                    {aiForm.provider === 'gemini' ? (
                      <>
                        <option value="gemini-3.5-flash-lite">gemini-3.5-flash-lite (Mặc định - Nhanh &amp; Nhẹ)</option>
                        <option value="gemini-3.6-flash">gemini-3.6-flash (Bản đầy đủ)</option>
                        <option value="gemini-3.5-flash">gemini-3.5-flash (Bản Flash 3.5)</option>
                        <option value="gemini-3.0-flash">gemini-3.0-flash (Bản Flash 3.0)</option>
                        <option value="gemini-3.6-pro">gemini-3.6-pro (Suy luận sâu)</option>
                      </>
                    ) : (
                      <>
