...
import { AdminDragHandle, buildOrderUpdates, getNextSortState, getReorderedItems, SearchInput, SelectCheckbox, SortableHeader, SortableTableRow, useAdminDndSensors, useSortableData } from '../../components/TableUtilities';
import type { DragEndEvent } from '@dnd-kit/core';
import { DndContext, closestCenter } from '@dnd-kit/core';
import { SortableContext, verticalListSortingStrategy } from '@dnd-kit/sortable';

export default function ResourceFiltersListPage() {
  return (
    <ModuleGuard moduleKey="resources">
      <ResourceFiltersContent />
    </ModuleGuard>
  );
}

function ResourceFiltersContent() {
  const router = useRouter();
  const filtersData = useQuery(api.resourceFilters.listAll, {});
  const deleteFilter = useMutation(api.resourceFilters.remove);
  const reorderFilters = useMutation(api.resourceFilters.reorder);

  const [searchTerm, setSearchTerm] = useState('');
  const [sortConfig, setSortConfig] = useState<{ key: string | null; direction: 'asc' | 'desc' }>({ direction: 'asc', key: 'order' });
  const [selectedIds, setSelectedIds] = useState<Id<'resourceFilters'>[]>([]);
  const [deleteTargetId, setDeleteTargetId] = useState<Id<'resourceFilters'> | null>(null);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [isDeleteLoading, setIsDeleteLoading] = useState(false);

  const deleteInfo = useQuery(api.resourceFilters.getDeleteInfo, deleteTargetId ? { id: deleteTargetId } : 'skip');
  ...
  const toggleSelectItem = (id: Id<'resourceFilters'>) => {
    setSelectedIds((prev) => prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]);
  };

  const handleDeleteClick = (id: Id<'resourceFilters'>) => {
      ...
      <div className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Bộ lọc tài nguyên</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">Quản lý các bộ lọc phần mềm / công cụ liên quan đến tài nguyên</p>
            </div>
          </div>
          <Button onClick={() => router.push('/admin/resources/filters/create')} className="gap-2 bg-indigo-600 hover:bg-indigo-500 text-white">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <SortableHeader label="Tên bộ lọc" sortKey="name" sortConfig={sortConfig} onSort={(key) => { setSortConfig((prev) => getNextSortState(prev, key)); }} />
                <SortableHeader label="Slug" sortKey="slug" sortConfig={sortConfig} onSort={(key) => { setSortConfig((prev) => getNextSortState(prev, key)); }} />
                <SortableHeader label="Số giá trị" sortKey="valuesCount" sortConfig={sortConfig} onSort={(key) => { setSortConfig((prev) => getNextSortState(prev, key)); }} />
                ...
                <SortableTableRow key={filter.id} id={filter.id} disabled={!isReorderEnabled} selected={selectedIds.includes(filter.id)} selectedClassName="bg-indigo-500/5">
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => router.push(`/admin/resources/filters/${filter.id}/edit`)} title="Chỉnh sửa">
                        <Edit size={16} />
                      </Button>
