...
import { Button, Input, Label, Badge, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, cn } from '@/app/admin/components/ui';
...
export default function ResourceFilterEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <ModuleGuard moduleKey="resources">
      <ResourceFilterEditContent id={id as Id<'resourceFilters'>} />
    </ModuleGuard>
  );
}

function ResourceFilterEditContent({ id }: { id: Id<'resourceFilters'> }) {
  const router = useRouter();

  // Queries & Mutations
  const filterData = useQuery(api.resourceFilters.getById, { id: id as Id<'resourceFilters'> });
  useSetAdminBreadcrumb(filterData?.name);
  const updateFilter = useMutation(api.resourceFilters.update);

  const filterValues = useQuery(api.resourceFilters.listValuesByFilter, { filterId: id });
  const createValue = useMutation(api.resourceFilters.createValue);
  ...
  const [editingValueId, setEditingValueId] = useState<Id<'resourceFilterValues'> | null>(null);
  ...
  const handleDeleteValue = async (valId: Id<'resourceFilterValues'>, valName: string) => {
    if (!confirm(`Bạn có chắc chắn muốn xóa giá trị "${valName}"? Các liên kết với tài nguyên cũng sẽ bị gỡ bỏ.`)) {
      return;
    }
    try {
  ...
  return (
    <AdminFormPageWrapper
      ...
      backHref="/admin/resources/filters"
      ...
      stickyFooter={
        <AdminStickyFooter
          ...
          onCancel={() => router.push('/admin/resources/filters')}
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="val-slug"
                  value={valSlug}
                  onChange={(e) => setValSlug(generateSlugFromTitle(e.target.value))}
