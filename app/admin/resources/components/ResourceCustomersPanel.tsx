...
import { AdminPagination, FilterSelect, generatePaginationItems, ResetFilterButton, SearchInput } from '../../components/TableUtilities';
      ...
      <Card>
        <div className="flex flex-col gap-4 border-b border-slate-100 p-4 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {showResourceColumn && allResources && (
              <FilterSelect
                value={selectedResourceId === 'all' ? '' : selectedResourceId}
                onChange={(val) => { setSelectedResourceId(val || 'all'); setCurrentPage(1); }}
                placeholder="Tất cả tài nguyên"
                options={allResources.map((res) => ({ value: res._id, label: res.title }))}
              />
            )}
            <FilterSelect
              value={status}
              onChange={(val) => { setStatus(val as typeof status); setCurrentPage(1); }}
            ...
            />
            <ResetFilterButton isFiltered={Boolean(searchTerm.trim() || selectedResourceId !== 'all' || status !== 'all')} onReset={handleResetFilters} />
          </div>
        </div>
        ...
        {totalCount > 0 && !isTableLoading && (
          <AdminPagination
            ...
            entityLabel="khách hàng nhận tài nguyên"
          />
        )}
      </Card>
