...
import { ExternalLink, Sparkles, Plus, Trash, Trash2 } from 'lucide-react';
...
function ProductCreateContent() {
  ...
  useEffect(() => {
    if (defaultStatus) {
      setStatus(defaultStatus as 'Draft' | 'Active');
    }
  }, [defaultStatus]);
            ...
            {seoTab === 'content' ? (
              ...
              <div className={enabledFields.has('sku') ? "grid grid-cols-2 gap-4" : ""}>
                <AdminSlugInput
                  slug={slug}
                  onChange={setSlug}
                  categorySlug={categoriesData?.find(c => c._id === categoryId)?.slug}
                />
            ...
            <AdminFormCard title="Render nâng cao">
                ...
                {hasHtmlRender && (
                  <div className="space-y-2">
                    ...
                    <textarea
                      value={htmlRender}
                      onChange={(e) =>{  setHtmlRender(e.target.value as 'content' | 'markdown' | 'html'); }}
                      className="w-full min-h-[120px] rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm font-mono"
