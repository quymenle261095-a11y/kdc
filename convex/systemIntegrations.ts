...
const aiConfigDoc = v.object({
  adminDirectAiEnabled: v.boolean(),
  enabled: v.boolean(),
  hasApiKey: v.boolean(),
...
function normalizeConfig(settings: Record<string, unknown>, hasApiKey: boolean, maskedApiKey?: string) {
  ...
  const config = {
    adminDirectAiEnabled: toBooleanValue(settings.ai_admin_direct_enabled, false),
...
export const getPublicAiConfig = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      adminDirectAiEnabled: config.adminDirectAiEnabled,
...
export const saveAiConfig = mutation({
  args: {
    adminDirectAiEnabled: v.optional(v.boolean()),
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all([
      ...
      upsertSetting(ctx, "ai_admin_direct_enabled", Boolean(args.adminDirectAiEnabled)),
