  ...
  if (slot.key === 'about') {
    sections.push(
      ...
      },
      {
        callout: resolvedContact,
        paragraphs: [
          'Hãy kết nối trực tiếp với chúng tôi để cập nhật những sản phẩm mới nhất, nhận thông tin ưu đãi độc quyền hoặc gửi yêu cầu tư vấn nhanh từ đội ngũ chuyên nghiệp:',
        ],
...
const buildAutoGeneratePlan = async (ctx: QueryCtx | MutationCtx) => {
  ...
  const existingPages = await PagesModel.listAll(ctx);
  const pageMap = new Map(existingPages.map((p) => [p.key, p]));
  ...
  const slots = TRUST_PAGE_SLOTS.map((slot) => {
    ...
    if (page) {
      return {
        ...
        enabled,
        pageId: page._id,
        postStatus: page.status,
        postTitle: page.title,
        slot,
      };
    }

    const payload = buildDraftPayload(slot, settingsMap);
    return {
  ...
  handler: async (ctx) => {
    ...
    return {
      slots: plan.slots.map((slot) => ({
        ...
        pageId: 'pageId' in slot && slot.pageId ? (slot.pageId as Id<'pages'>) : null,
        postId: null,
        postStatus: 'postStatus' in slot ? slot.postStatus ?? null : null,
        postTitle: 'postTitle' in slot ? slot.postTitle ?? null : null,
        slug: slot.slot.slug,
      })),
    };
  },
...
export const applyAutoGenerate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const slot of plan.slots) {
      if (!slot.enabled) {
        results.push({ action: 'disabled', key: slot.slot.key });
        disabledCount += 1;
        continue;
      }

      if (overwrite) {
        const payload = buildDraftPayload(slot.slot, plan.settingsMap);
        const pageId = await PagesModel.upsertByKey(ctx, {
          ...
          publishedAt: desiredStatus === 'Published' ? Date.now() : undefined,
          renderType: 'content',
          slug: slot.slot.slug,
      ...
      if (slot.action === 'mapped') {
        results.push({
          action: slot.action,
          key: slot.slot.key,
          pageId: 'pageId' in slot ? (slot.pageId as Id<'pages'>) : null,
          postId: null,
          postStatus: 'postStatus' in slot ? slot.postStatus : null,
          postTitle: 'postTitle' in slot ? slot.postTitle : null,
        });
        keptCount += 1;
        continue;
      }

      if ((slot.action === 'draft' || slot.action === 'suggested') && 'payload' in slot && slot.payload) {
        const pageId = await PagesModel.upsertByKey(ctx, {
    ...
    // Cập nhật timestamp lần autogen cuối cùng
    const existingSetting = await ctx.db
      .query('settings')
      .withIndex('by_key', (q) => q.eq('key', 'trust_page_last_autogen_at'))
      .unique();

    if (existingSetting) {
      await ctx.db.patch(existingSetting._id, { group: IA_GROUP, value: Date.now() });
    } else {
      await ctx.db.insert('settings', { group: IA_GROUP, key: 'trust_page_last_autogen_at', value: Date.now() });
    }
    ...
    return {
      ...
      updatedSettings: {
        trust_page_last_autogen_at: Date.now(),
      },
    };
  },
  returns: v.object({
