...
import { evaluateCouponHelper, updatePromotionStats } from "./promotions";
...
async function handleOrderStatusTransition(
  ...
  // Xử lý commit / release reservation coupon khi đổi trạng thái đơn
  ...
  if (reservation) {
      ...
      if (isCancelledStatus(newStatus)) {
        ...
        if (promotion) {
          await ctx.db.patch(promotion._id, {
            ...
            reservedCount: Math.max(0, (promotion.reservedCount ?? 1) - 1),
          });
        }
      } else {
        const lowerNewStatus = newStatus.toLowerCase();
        if (
          lowerNewStatus.includes("confirm") ||
          lowerNewStatus.includes("deliver") ||
          lowerNewStatus.includes("complete") ||
          lowerNewStatus.includes("xác nhận") ||
          lowerNewStatus.includes("hoàn thành")
        ) {
          await ctx.db.patch(reservation._id, { status: "committed" });
          if (promotion) {
            await ctx.db.patch(promotion._id, {
              budgetUsed: (promotion.budgetUsed ?? 0) + reservation.reservedDiscountAmount,
              reservedBudget: Math.max(
                0,
                (promotion.reservedBudget ?? reservation.reservedDiscountAmount) - reservation.reservedDiscountAmount
              ),
    ...
    } else if (reservation.status === "committed" && isCancelledStatus(newStatus)) {
      // QA-MED-01 FIX: Hoàn trả budgetUsed và usedCount khi đơn hàng đã confirmed bị hủy
      await ctx.db.patch(reservation._id, { status: "released" });
      if (promotion) {
        await ctx.db.patch(promotion._id, {
          budgetUsed: Math.max(0, (promotion.budgetUsed ?? 0) - reservation.reservedDiscountAmount),
          usedCount: Math.max(0, promotion.usedCount - 1),
        });
        await updatePromotionStats(ctx, "totalUsed", -1);
      }
    } else if (reservation.status === "released" && !isCancelledStatus(newStatus)) {
      const lowerNewStatus = newStatus.toLowerCase();
      if (
        lowerNewStatus.includes("confirm") ||
        lowerNewStatus.includes("deliver") ||
        ...
        // QA-HIGH-02 FIX: Tái commit lại voucher khi Admin khôi phục đơn từ Cancelled -> Confirmed
        await ctx.db.patch(reservation._id, { status: "committed" });
        if (promotion) {
          await ctx.db.patch(promotion._id, {
            budgetUsed: (promotion.budgetUsed ?? 0) + reservation.reservedDiscountAmount,
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // QA-CRIT-01 FIX: Trừ tồn kho sản phẩm / variant khi đặt đơn thành công
    if (stockCheckEnabled) {
    ...
    if (args.promotionCode?.trim()) {
      const codeClean = args.promotionCode.trim().toUpperCase();
      ...
      if (promotion && promotion.status === "Active") {
        ...
        const evalResult = await evaluateCouponHelper(ctx, {
          cartItems: normalizedItems
            .filter((item): item is typeof item & { productId: NonNullable<typeof item.productId> } => Boolean(item.productId))
            .map((item) => ({
              brandId: (item as unknown as Record<string, string>).brandId,
              categoryId: (item as unknown as Record<string, string>).categoryId,
              price: item.price,
              productId: item.productId,
              quantity: item.quantity,
              tags: (item as unknown as Record<string, string[]>).tags,
            })),
          code: codeClean,
          customerId: args.customerId,
        ...
        if (evalResult.isValid) {
          ...
          const reservedDiscountAmount = evalResult.discountAmount + evalResult.shippingDiscount;
          const expiresAt = now + 15 * 60 * 1000;

          // QA-HIGH-01 FIX: Cập nhật bản ghi cũ nếu đã tồn tại thay vì insert trùng orderId
          if (existingReservation) {
            await ctx.db.patch(existingReservation._id, {
              ...
              promotionId: promotion._id,
              reservedAt: now,
              reservedDiscountAmount,
              status: "reserved",
            });
          } else {
            await ctx.db.insert("promotionReservations", {
              customerId: args.customerId,
              expiresAt,
              orderId,
              promotionId: promotion._id,
          ...
          await ctx.db.patch(promotion._id, {
            reservedBudget: (promotion.reservedBudget ?? 0) + reservedDiscountAmount,
